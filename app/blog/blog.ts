...
async function requireSession(): Promise<{ session: Session; userId: string; isAdmin: boolean }> {
...
export async function createPost(input: {
  title: string; subtitle?: string; excerpt: string; content: string; coverUrl?: string; tags?: string[];
  status?: "DRAFT" | "PUBLISHED" | "SCHEDULED"; publishedAt?: Date | null
}) {
...
export async function updatePost(id: string, input: {
  title?: string; subtitle?: string | null; excerpt?: string; content?: string; coverUrl?: string | null;
  tags?: string[]; status?: "DRAFT" | "PUBLISHED" | "SCHEDULED"; publishedAt?: Date | null
}) {
  await requireAdmin()
  const data: any = { ...input }
  if (input.title) data.slug = slugify(input.title)
  if (typeof input.content === "string") data.readingTime = calcReadingTime(input.content)
...
export async function getPosts(input?: { tag?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, input?.page ?? 1)
  const pageSize = Math.min(20, Math.max(1, input?.pageSize ?? 10))
  const where: any = { status: "PUBLISHED" as const }
  if (input?.tag) where.tags = { some: { tag: { slug: input.tag } } }
...
async function findRootId(commentId: string): Promise<string> {
  let cur: { id: string; parentId: string | null; rootId: string | null } | null =
...
export async function addPostComment(input: {
  postId: string; message: string; parentId?: string | null; parentAuthor?: string | null
}) {
  ...
  if (input.parentAuthor) {
    const u = await prisma.user.findFirst({ where: { name: input.parentAuthor }, select: { id: true } })
  ...
  const slugObj = await prisma.post.findUnique({ where: { id: input.postId }, select: { slug: true } })
