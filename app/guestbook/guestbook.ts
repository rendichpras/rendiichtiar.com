...
export async function getGuestbookEntries(): Promise<PublicEntry[]> {
  ...
  const roots = await prisma.guestbook.findMany({
    ...
    include: {
      user: {
        select: { name: true, email: true, image: true, accounts: { select: { provider: true }, take: 1 } },
      },
  ...
  // ambil SEMUA reply milik root tersebut
...
export async function addGuestbookEntry(
  ...
  const created = await prisma.guestbook.create({
    data: {
      message: text,
      userId: user.id,
      parentId: parentId ?? null, // simpan induk langsung
      rootId,                      // simpan induk akar
      mentionedUserId,
    },
    select: { id: true },
  })
  ...
  if (parentId) {
    ...
    const e = await prisma.guestbook.findUnique({
      where: { id: created.id },
      include: {
        user: { select: { name: true, email: true, image: true, accounts: { select: { provider: true }, take: 1 } } },
        likes: { include: { user: { select: { name: true, email: true, image: true } } } },
      },
    })
    if (e) {
...
export async function toggleLike(guestbookId: string, userEmail: string) {
  ...
  if (existing) {
    ...
    emitEvent({ type: "guestbook:like", id: guestbookId, userEmail, action: "unlike" })
  } else {
    await prisma.like.create({ data: { userId: user.id, guestbookId } })
    emitEvent({ type: "guestbook:like", id: guestbookId, userEmail, action: "like" })
  }
